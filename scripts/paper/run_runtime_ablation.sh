...
NUM_GPUS="${NUM_GPUS:-8}"
MAX_TRAIN_STEPS="${MAX_TRAIN_STEPS:-250}"
...
for variant in eager cache_only cache_async cache_async_fused cache_async_fused_compile; do
  for run_seed in "${SEEDS[@]}"; do
    ...
    run_cmd bash "${PROJECT_ROOT}/scripts/run_train.sh" \
        ...
        --output_dir "${OUTPUT_ROOT}/${variant}/seed${run_seed}" \
        --gpus "${NUM_GPUS}" \
        --steps 4 \
        --batch_size 1 \
        --lr 1e-5 \
        ...
        "${extra_args[@]}"
    run_cmd python3 "${PROJECT_ROOT}/tools/summarize_run.py" \
        "${OUTPUT_ROOT}/${variant}/seed${run_seed}" --warmup 50 --measured 200
  done
done
