...
def apply_model_metadata(args: Any) -> dict[str, Any]:
    teacher_path = getattr(args, "teacher_model_path", "") or getattr(args, "model_path", "") or ""
    task = getattr(args, "task", None)
    metadata = resolve_model_metadata(getattr(args, "model_cls", ""), model_path=teacher_path, task=task)
    setattr(args, "model_cls", metadata["canonical_model_cls"])
    setattr(args, "resolved_runner_cls", metadata["runner_cls"])
    setattr(args, "model_architecture", metadata["architecture"])
    setattr(args, "model_family", metadata["model_family"])
    setattr(args, "model_status", metadata["status"])
    setattr(args, "checkpoint_format", metadata["checkpoint_format"])
    setattr(args, "supported_tasks", metadata["tasks"])
    setattr(args, "model_modalities", metadata["modalities"])
    setattr(args, "input_modalities", metadata["input_modalities"])
    setattr(args, "output_modalities", metadata["output_modalities"])
    setattr(args, "primary_modality", metadata["primary_modality"])
    setattr(args, "distill_stage", metadata["distill_stage"])
