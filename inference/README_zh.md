...
> **上游文档说明：** 以下性能与质量数字保留自 LightX2V 上游项目，
> WorldDistill 维护者尚未复现；它们不构成 WorldDistill CUDA 兼容性或
> benchmark 矩阵的验证证据。
>
> **许可边界：** 此 vendored 目录随仓库发布的代码采用 Apache-2.0。
> WorldDistill 不捆绑 Tencent HY-WorldPlay 源码或权重；可选的用户自备
> provider 仍受其独立条款约束。详见仓库的[第三方声明](../THIRD_PARTY_NOTICES)。
