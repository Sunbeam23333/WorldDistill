...
class WanModel(BaseTransformerModel):
    ...
    def infer(self, inputs):
        is_moe_model = "wan2.2_moe" in self.config["model_cls"] or "lingbot_cam_moe" in self.config["model_cls"]
        if self.cpu_offload:
            if self.offload_granularity == "model" and self.scheduler.step_index == 0 and not is_moe_model:
                self.to_cuda()
        ...
        if self.cpu_offload:
            if self.offload_granularity == "model" and self.scheduler.step_index == self.scheduler.infer_steps - 1 and not is_moe_model:
                self.to_cpu()
            elif self.offload_granularity != "model":
