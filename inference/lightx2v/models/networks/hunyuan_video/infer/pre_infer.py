...
def attention(
    q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, drop_rate: float = 0.0, attn_mask: Optional[torch.Tensor] = None, causal: bool = False, attn_type: str = "torch_sdpa"
) -> torch.Tensor:
    """
    ...
    if attn_type == "flash_attn2":
        ...
        x = sage_attn_no_pad_v3(qkv, attn_mask, causal=causal, dropout_p=drop_rate, softmax_scale=None)
    elif attn_type == "torch_sdpa":
        x = torch_sdpa_no_pad(qkv, attn_mask, causal=causal, dropout_p=drop_rate, softmax_scale=None)
    else:
        raise ValueError(f"Unsupported HunyuanVideo attention backend: {attn_type}")
    b, s, a, d = x.shape
    out = x.reshape(b, s, -1)
    return out
...
class HunyuanVideo15PreInfer:
    ...
    def run_individual_token_refiner(self, weights, out, mask, c):
        ...
        for block in weights.individual_token_refiner:  # block num = 2
            ...
            attn = attention(q, k, v, attn_mask=mask, attn_type=self.attn_type).squeeze(0)
