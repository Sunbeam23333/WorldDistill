    ...
    def __init__(self, config):
        ...
        if self.config.get("dit_quantized", False) and self.config.get("high_noise_quantized_ckpt", None):
            ...
            if not os.path.isdir(self.high_noise_model_path):
                # Fallback: Diffusers format uses transformer/ instead of high_noise_model/
                diffusers_path = os.path.join(self.config["model_path"], "transformer")
                if os.path.isdir(diffusers_path):
                    self.high_noise_model_path = diffusers_path
                else:
                    raise FileNotFoundError(f"High Noise Model not found at {self.high_noise_model_path} or {diffusers_path}")
        ...
        if self.config.get("dit_quantized", False) and self.config.get("low_noise_quantized_ckpt", None):
            ...
            if not os.path.isdir(self.low_noise_model_path):
                # Fallback: Diffusers format uses transformer_2/ instead of low_noise_model/
                diffusers_path = os.path.join(self.config["model_path"], "transformer_2")
                if os.path.isdir(diffusers_path):
                    self.low_noise_model_path = diffusers_path
                else:
                    raise FileNotFoundError(f"Low Noise Model not found at {self.low_noise_model_path} or {diffusers_path}")
