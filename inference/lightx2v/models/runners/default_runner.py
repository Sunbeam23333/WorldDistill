...
class DefaultRunner(BaseRunner):
    ...
    def init_modules(self):
        ...
        if self.config["task"] in {"i2v", "game"}:
    ...
    def load_vsr_model(self):
        if "video_super_resolution" in self.config:
            ...
            vsr_model_path = self.config["video_super_resolution"].get("model_path", "")
            if not vsr_model_path:
                raise ValueError(
                    "video_super_resolution is enabled but model_path is empty. "
                    "Set video_super_resolution.model_path to a local FlashVSR checkpoint "
                    "or remove the video_super_resolution block."
                )
            logger.info("Loading VSR model...")
            return VSRWrapper(vsr_model_path)
        else:
            return None
