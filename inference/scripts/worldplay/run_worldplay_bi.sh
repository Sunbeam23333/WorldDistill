...
BI_ACTION_MODEL_PATH="${BI_ACTION_MODEL_PATH:?Set BI_ACTION_MODEL_PATH to the WorldPlay BI action checkpoint}"
...
# Output
OUTPUT_PATH="${OUTPUT_PATH:-${LIGHTX2V_PATH}/save_results/HY-WorldPlay/worldplay_bi_test.mp4}"
...
python -m lightx2v.infer \
    ...
    --config_json "${LIGHTX2V_PATH}/configs/worldplay/worldplay_bi_i2v_480p.json" \
    --prompt "$PROMPT" \
