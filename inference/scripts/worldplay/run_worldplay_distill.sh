...
DISTILL_ACTION_MODEL_PATH="${DISTILL_ACTION_MODEL_PATH:?Set DISTILL_ACTION_MODEL_PATH to the distilled action checkpoint}"
...
# Output
OUTPUT_PATH="${OUTPUT_PATH:-${LIGHTX2V_PATH}/save_results/HY-WorldPlay/worldplay_distill_test.mp4}"
...
python -m lightx2v.infer \
    ...
    --config_json "${LIGHTX2V_PATH}/configs/worldplay/worldplay_distill_i2v_480p.json" \
    --prompt "$PROMPT" \
